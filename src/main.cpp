...
int main(int argc, char* argv[])
{
	...
	// ------------------------- Paramùtres de la simulation -------------------------



	Float	area = 500.;				// Taille de la zone d'apparition des ùtoiles (en annùes lumiùre)
	Float	galaxy_thickness = 0.05;	// Epaisseur de la galaxie (en "area")
	Float	precision = 0.5;			// Prùcision du calcul de l'accùlùration (algorithme de BarnesùHut)
	bool	verlet_integration = true;	// Utiliser l'intùgration de Verlet au lieu de la mùthode d'Euler

	int		stars_number = 30000;		// Nombre d'ùtoiles (Limitù ù 30 000 par les std::vector<>)
	Float	initial_speed = 2500.;		// Vitesse initiale des d'ùtoiles (en mùtres par seconde)
	Float	black_hole_mass = 0.;		// Masse du trou noir (en masses solaires)
	bool	is_black_hole = false;		// Prùsence d'un trou noir

	View	view = xy;		// Type de vue
	Float	zoom = 800.;				// Taille de "area" (en pixel)
	Float	real_colors = false;		// Activer la couleur rùelle des ùtoiles
	bool	show_blocks = false;		// Afficher les blocs

	Float	step = 200000.;				// Pas de temps de la simulation (en annùes)
	...
	while (time(NULL) < start + simulation_time) // Boucle du pas de temps de la simulation
	{
		...
		for (int i = 0; i < galaxy.size(); i++) // Boucle sur les ùtoiles de la galaxie
		{
